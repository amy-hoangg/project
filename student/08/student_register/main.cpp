...
void saveDataToFile(map<string, Student*> const &users,
                    string const &filename)
{
    ofstream file(filename);
    for(pair<string, Student*> const& pair_user : users)
    {
        Student *p = pair_user.second;
        string line = p->student_number + ";"
                     +p->user_id + ";"
                     +p->name + ";"
                     +p->phone_number + ";"
                     +p->email + ";"
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // find student
            if(student_numbers.find(parts.at(1))
                    == student_numbers.end())
            {
                cout << "There is no student with the given number!"<<endl<<endl;;
                continue; //pass this one and continue print register>
            }
            //student found
            //get phone number
            string phone;
